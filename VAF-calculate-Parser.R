library(magrittr)
x <- "T1_vs_N1_head.strelka.somatic.snvs.norm.vcf"
...
VAFs <- parse_vcf_alt1(x)
VAF_snvs<- dplyr::select(VAFs,CHROM,POS,REF,ALT,T_VAF, N_VAF)
write.table(VAF_snvs, "VAF_somatic.snvs.txt", sep = '\t')
